...
class DotsAndBoxesAgent:
    ...
    def next_action(self):
        ...
        # Adapt state to NN input
        feature_vector = self.change_state_dimension(copy.deepcopy(self.cells),
                                                     self.nb_cols+1)
        # Choose best action
        action = self.RL_agent.predict(feature_vector)
        r, c, o = action
        return r, c, o

    # Change dimension of state to be fed to the NN input
    def change_state_dimension(self, state, d):
        new_state = np.ndarray((d, d, 2))
